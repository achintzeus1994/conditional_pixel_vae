...
#                 dims = list(range(2, len(x[i].shape + 1))) # Not summing batch and Gibbs dims
#                 print(x_rec[i][0].shape, x[i].shape)#, dist.log_prob(x[i].unsqueeze(1)).shape)
                aux = -dist.log_prob(x[i].unsqueeze(1))
#                 print(aux.shape)
                negative_loglike.append(aux.sum(list(range(2, len(aux.shape)))))
